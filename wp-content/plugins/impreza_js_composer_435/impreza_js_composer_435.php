<?php
/*
Plugin Name: WPBakery Visual Composer 4.3.5 for Impreza
...
// Turning off impreza_js_composer or js_composer, if some of them is active
if (is_admin()){
...
class Vc_Manager {
	...
	private $plugin_name = 'impreza_js_composer_435/impreza_js_composer_435.php';
	/**
	...
	public function isNetworkPlugin() {
		if ( is_null( $this->is_network_plugin ) ) {
			...
			if ( is_multisite() && ( is_plugin_active_for_network( 'impreza_js_composer_435/impreza_js_composer_435.php' )
			  || is_network_only_plugin( 'impreza_js_composer_435/impreza_js_composer_435.php' ) )
			) {
	...
	public function assetUrl( $file ) {
		$plugins_url = plugins_url().'/'.dirname($this->pluginName()).'/'.$this->path( 'ASSETS_DIR_NAME', $file);
		return preg_replace( '/\s/', '%20', $plugins_url);
	}
}
/**
